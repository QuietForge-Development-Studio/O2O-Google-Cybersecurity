...
import { getModule, getModuleItems } from './SessionAPIUtils';
...
async function generateContent() {
  ...
  // 2. Fetch module and items
  const module = await getModule(COURSE_ID, MODULE_ID);
  // Assume getModule returns { name, items: [{ id, name, type, link }], linked: { onDemandCourseItemSubtitles: [...] }}

  // Build items array with subtopics from linked items
  ...
  // 3. Prepopulate notes & transcripts
  await generateBandFile(items, '2.4.1_notes.md');
  await generateBandFile(items, '2.4.1_transcripts.md');

  // 4. Handouts
  await fs.writeFile(
    path.join(OUTPUT_DIR, 'Handouts', '2.4_glossary.md'),
    '// Consolidated glossary entries\n'
  );
  await fs.writeFile(
